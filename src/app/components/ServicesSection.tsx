...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="bg-[#FAF9F6] py-12 lg:py-16">
      <div className="mx-auto max-w-350 px-4 sm:px-6 lg:px-10">
        ...
        {/* Expanding Cards */}
        <div className="flex h-175 w-full flex-col gap-3 sm:h-150 sm:gap-4 lg:h-137.5 lg:flex-row">
          {services.map((service: Service, index: number) => {
            ...
            return (
              <motion.div
                ...
                animate={{ flex: isActive ? 5 : 1.2 }}
                transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                className="group relative flex cursor-pointer flex-col overflow-hidden rounded-3xl bg-[#174440] min-h-22.5 lg:min-h-0"
              >
                {/* Background Image */}
                <motion.img
                  src={service.image}
                ...
                {/* Gradient Overlay */}
                <div
                  className={`absolute inset-0 bg-linear-to-t from-black/95 via-black/50 to-black/10 transition-opacity duration-500 ${
                ...
                {/* Dot */}
                <div
                  className={`absolute left-5 top-5 lg:left-8 lg:top-8 h-2.5 w-2.5 lg:h-3 lg:w-3 rounded-full ${service.dotColor} shadow-[0_0_10px_rgba(255,255,255,0.3)] z-20`}
                />
                ...
                <div className="relative z-10 flex h-full flex-col justify-end p-5 lg:p-8">
                  ...
                  >
                    <p className="mb-5 max-w-100 text-xs sm:text-sm leading-relaxed text-white/80 lg:text-base">
                      {service.description}
                    </p>

                    <Link
                      href="/services"
                      className="inline-flex items-center gap-2 rounded-md bg-[#D4B670] px-5 py-2.5 lg:px-6 lg:py-3 text-xs sm:text-sm font-semibold text-[#174440] transition-colors hover:bg-white"
                    >
                      Learn more
        ...
        <div className="mt-8 flex justify-center lg:mt-10">
          <Link
            href="/services"
            className="rounded-full border border-[#174440]/20 bg-transparent px-8 py-3 text-sm font-medium text-[#174440] transition-all hover:bg-[#174440]/5 hover:border-[#174440]/40"
          >
